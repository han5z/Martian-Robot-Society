...
class Citizen:
    ...
    def add_subordinate(self, subordinate: Citizen) -> None:
        ...
        >>> c1 = Citizen(4, "Starky Industries", 3024, "Labourer", 50)
        >>> c2 = Citizen(2, "Hookins National Lab", 3024, "Manager", 30)
        >>> c3 = Citizen(3, "S.T.A.R.R.Y Lab", 3010, "Commander", 60)
    ...
    def get_all_subordinates(self) -> List[Citizen]:
        ...
        >>> c4.get_all_subordinates() == [c1, c2, c3, c5, c6]
        True
        """
        ...
        if not self._subordinates:
            ...
            output = []
            for s in self._subordinates:
                temp = merge([s], s.get_all_subordinates())
    ...
    def get_society_head(self) -> Citizen:
        ...
        >>> c2 = Citizen(2, "Hookins National Lab", 3024, "Manager", 30)
        >>> c3 = Citizen(3, "S.T.A.R.R.Y Lab", 3010, "Commander", 60)
        >>> c4 = Citizen(4, "Oscorp", 3002, "CEO", 77)
        >>> c1.become_subordinate_to(c2)
        >>> c2.become_subordinate_to(c3)
    ...
    def get_closest_common_superior(self, cid: int) -> Citizen:
        ...
        True
        >>> c1.get_closest_common_superior(3) == c3
        True
        """
        # Note: This method must call itself recursively
        if self.get_citizen(cid) is not None or self.cid == cid:
            return self
        else:
    ...
    def get_district_name(self) -> str:
        ...
        elif self.get_superior() is None:
            return ''
        else:
    ...
    def rename_district(self, district_name: str) -> None:
        ...
        >>> c2 = DistrictLeader(2, "Hookins National Lab", 3024, "Manager", \
        30, "District A")
        >>> c3 = DistrictLeader(3, "Oscorp", 3004, "CEO", 77, "District X")
        >>> c1.become_subordinate_to(c2)
        >>> c2.become_subordinate_to(c3)
...
class Society:
    ...
    def add_citizen(self, citizen: Citizen, superior_id: int = None) -> None:
        ...
        True
        """
        if superior_id is None and self._head is None:
            self.set_head(citizen)
        elif superior_id is None and self._head is not None:
            previous_head = self._head
            self.set_head(citizen)
            previous_head.become_subordinate_to(self._head)
        else:
    ...
    def change_citizen_type(self, cid: int,
        ...
        >>> c2 = Citizen(2, "Hookins National Lab", 3024, "Manager", 65)
        >>> c3 = Citizen(3, "Starky Industries", 3024, "Labourer", 50)
        >>> o.add_citizen(c3, None)
        >>> o.add_citizen(c2, 3)
        >>> o.add_citizen(c1, 2)
        >>> isinstance(o.change_citizen_type(2, 'D6'), DistrictLeader)
        True
        >>> who = o.get_citizen(2)
        >>> isinstance(who, DistrictLeader)
        True
        >>> who.get_district_name() == 'D6'
        True
        >>> c1.get_superior() == who
        True
        >>> c3.get_direct_subordinates() == [who]
        True
        >>> c3.get_all_subordinates() == [c1, who]
        True
        >>> c2.get_superior() is None
        True
        >>> c2.get_all_subordinates()
        []
        """
        new_c = None
        current = self.get_citizen(cid)
        sup = current.get_superior()
        if isinstance(current, DistrictLeader):
            new_c = Citizen(current.cid, current.manufacturer,
                            current.model_year, current.job, current.rating)
        elif isinstance(current, Citizen):
            new_c = DistrictLeader(current.cid, current.manufacturer,
                                   current.model_year, current.job,
                                   current.rating, district_name)

        if self.get_head() == current:
            self.set_head(new_c)

        if sup is not None:
            sup.remove_subordinate(current.cid)
            self.add_citizen(new_c, sup.cid)

        for s in current.get_direct_subordinates():
            current.remove_subordinate(s.cid)
            self.add_citizen(s, new_c.cid)
        return new_c
    ...
    def _swap_up(self, citizen: Citizen) -> Citizen:
        ...
        >>> o = Society()
        >>> c1 = Citizen(1, "Starky Industries", 3024, "Worker", 50)
        >>> c2 = Citizen(2, "Hookins National Lab", 3024, "Manager", 65)
        >>> c3 = Citizen(3, "Oscorp", 3024, "LVL 100 B0ss", 50)
        >>> o.add_citizen(c3, None)
        >>> o.add_citizen(c2, 3)
        >>> o.add_citizen(c1, 2)
        >>> w = o._swap_up(c1)
        >>> w.get_direct_subordinates() == [o.get_citizen(2)]
        True
        >>> w.get_superior() == o.get_citizen(3)
        True
        >>> w.cid == 1
        True
        >>> w.job == "Manager"
        ...
        s.cid, s.manufacturer, s.model_year, s.rating, citizen.cid, \
            citizen.manufacturer, citizen.model_year, citizen.rating = \
            citizen.cid, citizen.manufacturer, citizen.model_year, \
            citizen.rating, s.cid, s.manufacturer, s.model_year, s.rating
        s.remove_subordinate(citizen.cid)
        s.add_subordinate(citizen)
        if s.get_superior() is not None:
            sup = s.get_superior()
            sup.remove_subordinate(s.cid)
            sup.add_subordinate(s)
        return s
    ...
    def promote_citizen(self, cid: int) -> None:
        ...
        >>> o = Society()
        >>> c1 = Citizen(1, "Starky Industries", 3024, "Worker", 70)
        >>> c2 = Citizen(2, "Hookins National Lab", 3024, "Manager", 65)
        >>> c3 = DistrictLeader(3, "Oscorp", 3024, "LVL 100 B0ss", 50, "D1")
        >>> o.add_citizen(c3, None)
        >>> o.add_citizen(c2, 3)
        >>> o.add_citizen(c1, 2)
        >>> o.promote_citizen(1)
        >>> len(o.get_all_citizens()) == 3
        True
        >>> o.get_head().cid == 1
        True
        >>> o.get_head().get_direct_subordinates()[0].cid == 3
        True
        >>> o.get_citizen(3).get_direct_subordinates()[0].cid == 2
        True
        """
        citizen = self.get_citizen(cid)
        while citizen.get_superior() is not None and \
                not isinstance(citizen, DistrictLeader) and \
    ...
    def delete_citizen(self, cid: int) -> None:
        ...
        >>> o.add_citizen(c1, 2)
        >>> o.delete_citizen(2)
        >>> len(o.get_all_citizens()) == 2
        True
        >>> o.get_citizen(3).get_direct_subordinates()[0].cid == 1
        True
        >>> o.delete_citizen(3)
        >>> len(o.get_all_citizens()) == 1
        True
        >>> o.get_head().cid == 1
        True
        """
        remove = self.get_citizen(cid)

        if remove == self.get_head():
            if remove.get_direct_subordinates():
                swap = remove.get_highest_rated_subordinate()
                self.set_head(swap)
                remove.remove_subordinate(swap.cid)
                for s in remove.get_direct_subordinates():
                    self._head.add_subordinate(s)
            else:
                self._head = None
        else:
            sup = remove.get_superior()
            sup.remove_subordinate(remove.cid)
            subs = remove.get_direct_subordinates()
            for s in subs:
                sup.add_subordinate(s)
